...
int startHangPoint=0;
int stopHangPoint=10;
...
void ComputeForces( ) {
	...
	for(i=0;i<total_points;i++) {
		...
		//add gravity force
//		if(i!=0 && i!=( numX)	)
if(i!=startHangPoint && i!=stopHangPoint)
		{
		F[i] += gravity ;
        }
	...
	for(i=0;i<springs.size();i++) {
		...
		if(springs[i].p1 != startHangPoint && springs[i].p1 != stopHangPoint)
			{F[springs[i].p1] += springForce;}
		if(springs[i].p2 != startHangPoint && springs[i].p2 != stopHangPoint )
...
void ApplyProvotDynamicInverse() {
	...
	for(size_t i=0;i<springs.size();i++) {
		...
		if(dist>springs[i].rest_length) {
			...
			} else if(springs[i].p2==startHangPoint|| springs[i].p2 ==stopHangPoint) {
